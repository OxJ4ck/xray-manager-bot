...
- 🎯 Entry points to execute your program using `python -m <xray_manager_bot>` or `$ xray_manager_bot` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/OxJ4ck/xray-manager-bot/branch/main/graph/badge.svg?token=xray-manager-bot_token_here)](https://codecov.io/gh/OxJ4ck/xray-manager-bot)
[![CI](https://github.com/OxJ4ck/xray-manager-bot/actions/workflows/main.yml/badge.svg)](https://github.com/OxJ4ck/xray-manager-bot/actions/workflows/main.yml)

Awesome xray_manager_bot created by OxJ4ck

## Install

from source
```bash
git clone https://github.com/OxJ4ck/xray-manager-bot xray_manager_bot
cd xray_manager_bot
make install
```
...
```bash
pip install xray_manager_bot
```

## Executing

```bash
$ xray_manager_bot run --port 8080
```

or

```bash
python -m xray_manager_bot run --port 8080
```

or

```bash
$ uvicorn xray_manager_bot:app
```

## CLI

```bash
❯ xray_manager_bot --help
Usage: xray_manager_bot [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ xray_manager_bot create-user --help
Usage: xray_manager_bot create-user [OPTIONS] USERNAME PASSWORD
...
```bash
xray_manager_bot create-user admin admin --superuser
```
...
Run with `xray_manager_bot run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `xray_manager_bot create-user`.
...
-----------------------------------------------------
xray_manager_bot/__init__.py              4      0   100%
xray_manager_bot/app.py                  16      1    94%
xray_manager_bot/cli.py                  21      0   100%
xray_manager_bot/config.py                5      0   100%
xray_manager_bot/db.py                   10      0   100%
xray_manager_bot/models/__init__.py       0      0   100%
xray_manager_bot/models/content.py       47      1    98%
xray_manager_bot/routes/__init__.py      11      0   100%
xray_manager_bot/routes/content.py       52     25    52%
xray_manager_bot/routes/security.py      15      1    93%
xray_manager_bot/routes/user.py          52     26    50%
xray_manager_bot/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in xray_manager_bot/default.toml.
